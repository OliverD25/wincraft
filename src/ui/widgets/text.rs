...
/// 13/18 for descriptions, verdicts and status.
pub fn secondary(ui: &mut Ui, text: &str, colour: Color32) -> Response {
    wrapped(ui, job(text, theme::regular(13.0), colour, Some(18.0)))
}

/// 12/16 for footer hints and the status line.
pub fn caption(ui: &mut Ui, text: &str, colour: Color32) -> Response {
    wrapped(ui, job(text, theme::regular(12.0), colour, Some(16.0)))
}

/// 13 secondary, at most 560 wide: the line under every page title.
...
/// Section heading: 13/18 semibold, sentence case, as written.
pub fn section_job(text: &str, colour: Color32) -> LayoutJob {
    job(text, theme::semibold(13.0), colour, Some(18.0))
}

pub fn group_header(ui: &mut Ui, text: &str) -> Response {
    let tokens = Tokens::get(ui.ctx());
    single(ui, section_job(text, tokens.text_secondary))
}

/// 28/34 semibold page title, with the mono meta line set 10 px after it on
/// the same baseline when there is one.
pub fn title(ui: &mut Ui, text: &str, meta: Option<&str>) -> Response {
    let tokens = Tokens::get(ui.ctx());
    let mut job = job(text, theme::semibold(28.0), tokens.text_primary, Some(34.0));
    if let Some(meta) = meta {
        // Its own line height and bottom alignment put the small mono text on
        // the title's baseline instead of at the top of the 34 px line.
...
pub fn link(ui: &mut Ui, text: &str) -> Response {
    ...
    let response = ui.add(
        Label::new(job(text, theme::regular(12.0), colour, Some(16.0)))
            .extend()
